    ...
    <div className='w-[1170px] h-[359px] mx-36  grid grid-cols-2 xl:grid-cols-2 lg:grid-cols-1 md:grid-cols-1 sm:grid-cols-1 xs:grid-cols-1' >
        ...
        <div className='flex flex-col pt-10 '>
            ...
            <div className='mt-10 '>
                ...
                <Link to='https://www.linkedin.com/in/santhosh-s-b51939287/'><CiLinkedin className='text-2xl cursor-pointer hover:text-green-500'/></Link>
                <Link to='https://github.com/santhoshpandy/'><FaGithub className='text-2xl cursor-pointer hover:text-green-500'/></Link>
                <FaSquareXTwitter className='text-2xl cursor-pointer hover:text-green-500' />
        ...
        <form action='' className='flex flex-col pt-32 '>
            <div className='relative'>
              <IoPerson className='absolute top-2 left-3 text-2xl text-green-400' />
               <input type="text" className='placeholder-black ring-2 ring-gray-400 border-r-2 w-96 h-10 pl-12' placeholder='Full Name' required/>
            </div>
            <div className='relative mt-5'>
                <MdOutlineMail className=' absolute top-2 left-3 text-green-400 text-2xl'/>
                <input type="email" className='placeholder-black ring-2 ring-gray-400 w-96 h-10 pl-12' name="" id="123" placeholder='Email' required/>
            </div>
            <button type='submit' className='bg-green-600 w-32 h-10 mt-10 text-white hover:text-green-500 hover:bg-white rounded-xl duration-700' >Submit</button>
        </form>
    </div>

      <div className='bg-black h-64 md:mt-80 sm:mt-96 xs:mt-[200px] relative'>
             <div className='flex justify-center gap-5 pt-20'>
                <FaFacebook className='text-white text-2xl cursor-pointer hover:text-green-500 '/>
                <FaYoutube className='text-white text-2xl cursor-pointer hover:text-green-500' />
                <AiFillBehanceCircle className='text-white text-2xl cursor-pointer hover:text-green-500 '/>
                <FaGoogle className='text-white text-2xl cursor-pointer hover:text-green-500' />
